...
from motep.grader.algorithms import find_active_set_exhaustive, find_active_set_maxvol
...
class Grader:
    ...
    def update(self, images: list[Atoms]) -> None:
        ...
        if self.algorithm == "exhaustive":
            self.indices = find_active_set_exhaustive(matrix)
        elif self.algorithm == "maxvol":
            self.indices = find_active_set_maxvol(
                matrix,
                self.rng,
