...
def _parse_radial_mag_coeffs(file: TextIO, data: dict) -> np.ndarray:
    ...
    shape = (
        ...
        data["species_count"],
        data["radial_funcs_count"],
        data["radial_basis_size"] * data["mag_basis_size"] * data["mag_basis_size"],
    )
    return np.array(coeffs).reshape(shape)
...
def read_mmtp(file: os.PathLike) -> MagMTPData:
    """Read an MLIP .mtp file."""
    data = {}
    with pathlib.Path(file).open("r", encoding="utf-8") as fd:
        for line in fd:
            ...
            if "=" in line:
                key, value = (_.strip() for _ in line.strip().split("="))
                if key in {"scaling", "min_dist", "max_dist", "min_mag", "max_mag"}:
                    data[key] = np.float64(value)
                elif value.isdigit():
                    data[key] = np.int32(value)
                elif key == "alpha_moment_mapping":
                ...
                key = "radial_coeffs"
                data[key] = _parse_radial_mag_coeffs(fd, data)
