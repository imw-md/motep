...
@pytest.mark.parametrize("level", [2, 4, 6, 8, 10])
def test_jac_mag_smoke(
    *,
    level: int,
    data_path: pathlib.Path,
) -> None:
    """Smoke-test that LossFunction.jac runs without error for magnetic potentials.

    Correctness of the engine-level Jacobian is tested in
    tests/potentials/mmtp/test_jacobian.py.
    """
    ...
    loss(mtp_data.parameters)
    jac = loss.jac(mtp_data.parameters)
    assert jac.shape == mtp_data.parameters.shape
